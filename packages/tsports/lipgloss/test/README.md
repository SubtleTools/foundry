...
Golden files use semantic naming that clearly indicates the color profile being tested:

- **`{test}_ascii.golden`** - Ascii profile (NO_COLOR=1, no color output)
- **`{test}_ansi.golden`** - ANSI profile (16 colors, FORCE_COLOR=0)
- **`{test}_ansi256.golden`** - ANSI256 profile (256 colors, FORCE_COLOR=1 or FORCE_COLOR=2)
- **`{test}_truecolor.golden`** - TrueColor profile (24-bit RGB, FORCE_COLOR=3)
...
This naming scheme makes it immediately clear which color mode each golden file represents, improving debugging and maintenance.

**See [GOLDEN_FILE_NAMING.md](./GOLDEN_FILE_NAMING.md) for detailed documentation on the golden file naming convention.**
