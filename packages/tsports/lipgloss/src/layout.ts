 ...
 */
export function parsePaddingShorthand(values: number[]): PaddingConfig {
  if (values.length === 0) {
 ...
 */
export function parseMarginShorthand(values: number[]): MarginConfig {
  // Reuse the padding parsing logic since they follow the same pattern
  return parsePaddingShorthand(values) as MarginConfig;
}
...
const GO_RUNEWIDTH_OVERRIDES: Record<string, number> = {
  ...
  '▶': 1, // Black right-pointing triangle - Extended Pictographic/Ambiguous but Go runewidth reports 1
  '🍥': 2, // Fish cake - Emoji, width 2
};
 ...
 */
export function getLineWidths(text: string): number[] {
  if (!text) return [0];
 ...
 */
export function getMaxTextWidth(text: string): number {
 ...
 */
export function getHorizontalPadding(padding?: PaddingConfig): number {
  if (!padding) return 0;
 ...
 */
export function getVerticalPadding(padding?: PaddingConfig): number {
  if (!padding) return 0;
 ...
 */
export function getHorizontalMargin(margin?: MarginConfig): number {
  if (!margin) return 0;
 ...
 */
export function getVerticalMargin(margin?: MarginConfig): number {
  if (!margin) return 0;
 ...
 * @returns Array of wrapped lines
 */
function wrapLineWordBased(line: string, maxWidth: number): string[] {
 ...
 * @returns Array of wrapped lines
 */
function wrapLineCharacterBased(line: string, maxWidth: number): string[] {
  const lines: string[] = [];
  let currentLine = '';
...
export function calculateHeight(
  heightConfig: HeightConfig | undefined,
 ...
 */
export function applyGoStyleWidthConstraint(text: string, targetWidth: number): string {
  if (targetWidth <= 0) return text;
 ...
 * @returns Array of wrapped lines
 */
export function wrapLineGoStyle(line: string, maxWidth: number): string[] {
  // For very narrow widths, Go uses character-based wrapping
  if (maxWidth <= 2) {
    return wrapLineCharacterBased(line, maxWidth);
  }

  // If the line already fits within maxWidth, no wrapping needed
  // Note: This handles the case where padding has already been applied to the content
  // and we don't want to break apart padded text unnecessarily
  if (getTextWidth(line) <= maxWidth) {
    return [line];
  }
  
  // Check if this is padded content that would fit without word breaking
  // This preserves leading/trailing whitespace that represents padding
  const trimmedLine = line.trim();
  const leadingSpace = line.match(/^ */)?.[0] || '';
  const trailingSpace = line.match(/ *$/)?.[0] || '';
  const contentWidth = getTextWidth(trimmedLine);
  
  // If the core content fits and we have leading/trailing space, 
  // this is likely padded content that shouldn't be broken apart
  if (contentWidth <= maxWidth && (leadingSpace || trailingSpace)) {
    return [line];
  }
  ...
  for (const word of words) {
    if (!currentLine) {
      // First word on line - add leading whitespace to first line only
      const prefixToUse = isFirstLine ? leadingWhitespace : '';
      const wordWithPrefix = prefixToUse + word;
      
      if (getTextWidth(wordWithPrefix) <= maxWidth) {
        currentLine = wordWithPrefix;
      } else if (isFirstLine && getTextWidth(word) <= maxWidth - leadingWhitespace.length) {
        // Leading whitespace makes it too long, but word alone would fit with reduced space
        currentLine = wordWithPrefix;
      } else {
        // Word is too long even without prefix, break it using character boundaries
        const brokenLines = wrapLineCharacterBased(word, maxWidth);
        // Add leading whitespace to first broken line only
        if (isFirstLine && brokenLines.length > 0) {
          brokenLines[0] = leadingWhitespace + brokenLines[0];
        }
        lines.push(...brokenLines.slice(0, -1)); // Add all but last
        currentLine = brokenLines[brokenLines.length - 1] || '';
        isFirstLine = false; // No longer first line after breaking
      }
      isFirstLine = false; // Mark that we're no longer on the first line
    } else {
      ...
      if (getTextWidth(testLine) <= maxWidth) {
        ...
        // If the word fits on a new line (and isn't huge), prefer wrapping it to a new line
        // rather than splitting it. Only split if the word implies it handles widths > maxWidth
        // or if we really want to fill every pixel (which Go's Reflow doesn't seemingly force by default).
        ...
        // Word doesn't fit on current line, AND implies it might be longer than maxWidth,
        // (or we just want to fill the current line aggressively?)
        // Actually, if word > maxWidth, we MUST split it.
        // We can try to fit part of it using Go's aggressive approach to fill the current line.

        const remainingSpace = maxWidth - getTextWidth(currentLine + ' ');
        
        // If we have reasonable remaining space, try to fit part of the word
        if (remainingSpace >= 2 && word.length > 3) {
          let bestFit = '';
          // Try to fit as much of the word as possible, but prefer shorter breaks
          // that leave reasonable remaining parts (like "go" + "ing")
          for (let i = 1; i <= word.length; i++) {
            const partialWord = word.substring(0, i);
            if (getTextWidth(partialWord) <= remainingSpace) {
              bestFit = partialWord;
              // If we have a good balance (partial word around 2-3 chars and remaining >= 3 chars), prefer it
              if (partialWord.length >= 2 && partialWord.length <= 3 && word.length - partialWord.length >= 3) {
                break; // Use this balanced split
              }
            } else {
              break;
            }
          ...
          // If we can fit a reasonable portion (at least 2 chars), use it
          if (bestFit.length >= 2) {
            currentLine = currentLine + ' ' + bestFit;
            lines.push(currentLine);
            currentLine = word.substring(bestFit.length);
          } else {
            // Can't fit meaningful part, start new line normally
            lines.push(currentLine);
            // We already know word > maxWidth, so break it
            const brokenLines = wrapLineCharacterBased(word, maxWidth);
            lines.push(...brokenLines.slice(0, -1)); // Add all but last
            currentLine = brokenLines[brokenLines.length - 1] || '';
          }
        } else {
           // Not enough space for partial fitting, start new line normally
          lines.push(currentLine);
          // We already know word > maxWidth, so break it
          const brokenLines = wrapLineCharacterBased(word, maxWidth);
          lines.push(...brokenLines.slice(0, -1)); // Add all but last
  ...
  // Go's behavior: if word-based wrapping leaves too much unused space,
  // fall back to character-based wrapping for better space utilization  
  // NOTE: This efficiency check is disabled as it conflicts with expected Go output
  // The Go reference shows word-based wrapping even with low efficiency
  /*
  if (lines.length > 0) {
    let totalUsed = 0;
    for (const wrappedLine of lines) {
      totalUsed += getTextWidth(wrappedLine);
    }
    const totalAvailable = lines.length * maxWidth;
    const efficiency = totalUsed / totalAvailable;
    
    // If efficiency is too low (< 70%), use character-based wrapping instead
    // Go prefers character-based wrapping for better space utilization
    if (efficiency < 0.7) {
      return wrapLineCharacterBased(line, maxWidth);
    }
  }
...
export function applyPadding(
  ...
  const maxContentWidth = leftPaddedLines.reduce(
    (max, lineInfo) => Math.max(max, lineInfo.contentWidth),
    0
  );

  // Calculate the target total width for all lines
  let targetTotalWidth: number;
  if (width !== undefined) {
      ...
      if (horizontalAlignment && contentSpaceNeeded > 0) {
        ...
        switch (horizontalAlignment) {
          ...
          case HorizontalAlignment.Center: {
            ...
            return (
              ' '.repeat(leftPadding + leftExtra) + original + ' '.repeat(rightExtra + rightPadding)
            );
          }
          case HorizontalAlignment.Right:
            return (
              ' '.repeat(leftPadding + contentSpaceNeeded) + original + ' '.repeat(rightPadding)
            );
          default:
            return leftPadded + ' '.repeat(contentSpaceNeeded + rightPadding);
        }
 ...
 */
export function applyMarginBackground(text: string, marginBackground: ColorValue): string {
