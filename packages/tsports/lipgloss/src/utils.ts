...
import { ColorProfile, type ColorValue } from './types';
import { Profile as TermenvProfile } from '@tsports/termenv';

/**
 * Position represents a position along a horizontal or vertical axis.
 * A value of 0 represents the start (left/top) and 1 represents the end (right/bottom).
 ...
 */
export function getLines(str: string): { lines: string[]; widest: number } {
  if (!str) return { lines: [''], widest: 0 };
...
/**
 * Whitespace rendering options for styling and customizing whitespace appearance.
 *
 * This interface defines options for rendering whitespace with custom colors and characters.
 * Used by whitespace rendering functions to control the visual appearance of spaces,
 * padding, and fill areas.
 *
 * @example
 * ```typescript
 * // Basic whitespace with colors
 * const redWhitespace: WhitespaceOptions = {
 *   foreground: 'red',
 *   background: 'yellow'
 * };
 *
 * // Custom whitespace characters
 * const dottedWhitespace: WhitespaceOptions = {
 *   chars: '·',
 *   foreground: 'gray'
 * };
 *
 * // Complex pattern whitespace
 * const patternWhitespace: WhitespaceOptions = {
 *   chars: '.-',
 *   foreground: 'blue',
 *   background: 'white'
 * };
 * ```
 *
 * @see {@link WhitespaceOption} For whitespace option functions
 * @see {@link WithWhitespaceForeground} For setting foreground color
 * @see {@link WithWhitespaceBackground} For setting background color
 * @see {@link WithWhitespaceChars} For setting custom characters
 */
export interface WhitespaceOptions {
  /** Foreground color for whitespace characters using any valid color format */
  foreground?: ColorValue;
  /** Background color for whitespace characters using any valid color format */
  background?: ColorValue;
  /** Characters to use for whitespace rendering (default: single space) */
  chars?: string;
}

/**
 * Function type for whitespace configuration options.
 *
 * WhitespaceOption functions modify a WhitespaceOptions object to configure
 * how whitespace should be rendered. These functions are created by the
 * `With*` helper functions and can be composed to build complex whitespace styling.
 *
 * @example
 * ```typescript
 * // Create whitespace options
 * const options: WhitespaceOptions = {};
 *
 * // Apply configuration functions
 * const setRed = WithWhitespaceForeground('red');
 * const setDots = WithWhitespaceChars('·');
 *
 * setRed(options);
 * setDots(options);
 *
 * // options now has: { foreground: 'red', chars: '·' }
 *
 * // Or chain multiple options
 * const applyOptions = (opts: WhitespaceOptions) => {
 *   WithWhitespaceForeground('blue')(opts);
 *   WithWhitespaceBackground('yellow')(opts);
 *   WithWhitespaceChars('*')(opts);
 * };
 * ```
 *
 * @param options - The WhitespaceOptions object to modify
 *
 * @see {@link WhitespaceOptions} For the options interface
 * @see {@link WithWhitespaceForeground} For foreground color options
 * @see {@link WithWhitespaceBackground} For background color options
 * @see {@link WithWhitespaceChars} For character options
 */
export type WhitespaceOption = (options: WhitespaceOptions) => void;
 ...
 */
export function renderWhitespace(width: number, options: WhitespaceOptions = {}): string {
  if (width <= 0) return '';

  const { chars = ' ', foreground, background } = options;

  if (!chars) return ' '.repeat(width);

  const runes = Array.from(chars);
  if (runes.length === 0) return ' '.repeat(width);
  ...
  // We add characters only if they don't exceed the target width.
  while (currentWidth < width) {
    const rune = runes[runeIndex];
    const runeWidth = getTextWidth(rune);

    // Don't add this rune if it would exceed the target width
  ...
  // Fill any remaining gaps with spaces. This might be necessary if any runes
  // are more than one cell wide, which could leave a gap.
  const short = width - currentWidth;
  if (short > 0) {
    result += ' '.repeat(short);
  }
  ...
  if (foreground || background) {
    let style = new Style();
    if (foreground) style = style.color(foreground);
 ...
 */
export function WithWhitespaceForeground(color: ColorValue): WhitespaceOption {
  return (options: WhitespaceOptions) => {
 ...
 * @see {@link Place} - For positioning text with styled whitespace fill
 */
export function WithWhitespaceBackground(color: ColorValue): WhitespaceOption {
  return (options: WhitespaceOptions) => {
 ...
 */
export function getFirstRune(str: string): string {
  if (!str) return '';
  const runes = Array.from(str);
  return runes[0] || '';
}
 ...
 */
export function toRunes(str: string): string[] {
 ...
 */
function initializeGlobalColorProfile(): ColorProfile {
...
let _colorProfile: ColorProfile = initializeGlobalColorProfile();
let _hasDarkBackground: boolean | null = null;

// NOTE: Removed explicit color profile setting during initialization.
// The renderer should use environment-based detection unless explicitly overridden.
// This allows environment variables like FORCE_COLOR=0 to be respected by tests.
 ...
 */
function lipglossToTermenvProfile(lipglossProfile: ColorProfile): TermenvProfile {
 ...
 * Now that enums are aligned, this is a direct cast!
 */
function termenvToLipglossProfile(termenvProfile: TermenvProfile): ColorProfile {
  // ColorProfile enum now matches TermenvProfile exactly - direct conversion
  return termenvProfile as unknown as ColorProfile;
}
...
export function SetColorProfile(profile: ColorProfile): void {
  ...
  try {
    ...
    // Pass the original lipgloss ColorProfile instead of converted termenv Profile
    // The renderer can handle the conversion internally
    rendererSetColorProfile(profile);
  } catch (error) {
