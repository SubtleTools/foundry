 ...
 */
export function alignTextHorizontal(
  str: string,
  pos: import('./position-types').LipglossPosition,
  width: number,
  style?: import('@tsports/termenv').Style
): string {
 ...
 */
export function alignTextVertical(
  str: string,
  pos: import('./position-types').LipglossPosition,
  height: number,
  style?: import('@tsports/termenv').Style
...
export class AlignUtils {
  ...
  static alignVertical(
    ...
    if (lines.length === 0 || height <= 0) return lines;
  ...
  static center(text: string, width: number, height: number): string[] {
    ...
    const hAligned = lines.map((line) =>
      AlignUtils.alignHorizontal(line, width, HorizontalAlignment.Center)
    );
  ...
  static justify(text: string, width: number): string {
    ...
    for (let i = 0; i < words.length; i++) {
      const word = words[i];
      if (word === undefined) break; // Type safety for noUncheckedIndexedAccess
   ...
   */
  static wrapText(text: string, width: number, breakWords: boolean = false): string[] {
    // Input validation - handle negative dimensions gracefully
    if (!Number.isFinite(width) || width < 0) {
      return [text];
    }
  ...
  static alignTextBox(
    ...
    if (!text) return Array(Math.max(0, height)).fill(' '.repeat(Math.max(0, width)));
    ...
    const hAligned = lines.map((line) =>
      AlignUtils.alignHorizontal(line, width, horizontalAlignment)
    );
