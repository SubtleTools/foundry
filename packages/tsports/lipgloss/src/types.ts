...
// Forward declaration for TerminalColor interface
export interface TerminalColor {
   ...
   */
  color(renderer: any): any; // Using any to avoid circular dependency
...
export type ColorValue =
  | string // Hex colors like "#ff0000" or "#f00", RGB strings like "rgb(255,0,0)", or named colors
  | NamedColor // Named colors like "red", "blue"
  | RGBColor // RGB object { r: 255, g: 0, b: 0 }
  | RGBAColor // RGBA object { r: 255, g: 0, b: 0, a: 1 }
  | HSLColor // HSL object { h: 0, s: 100, l: 50 }
  | ANSIColor // ANSI 256-color codes (0-255)
  | TerminalColor // Go-compatible color instances with deferred resolution
...
export interface StyleProperties {
  ...
  // Additional properties used in Style class
  /** @deprecated Use padding.top */
  paddingTop?: number;
  /** @deprecated Use padding.right */
  paddingRight?: number;
  /** @deprecated Use padding.bottom */
  paddingBottom?: number;
  /** @deprecated Use padding.left */
  paddingLeft?: number;
  /** @deprecated Use margin.top */
  marginTop?: number;
  /** @deprecated Use margin.right */
  marginRight?: number;
  /** @deprecated Use margin.bottom */
  marginBottom?: number;
  /** @deprecated Use margin.left */
  marginLeft?: number;
  /** @deprecated Use border.style */
  borderStyle?: BorderType | BorderStyle;
  /** @deprecated Use border.top */
  borderTop?: boolean;
  /** @deprecated Use border.right */
  borderRight?: boolean;
  /** @deprecated Use border.bottom */
  borderBottom?: boolean;
  /** @deprecated Use border.left */
  borderLeft?: boolean;
...
export enum ColorProfile {
  /** True color 24-bit RGB support (16.7 million colors) - matches termenv.TrueColor */
  TrueColor = 0,
  /** Extended 256 color support (includes RGB cube and grayscale) - matches termenv.ANSI256 */
  ANSI256 = 1,
  /** Basic 16 color support (standard ANSI colors) - matches termenv.ANSI */
  ANSI = 2,
  /** No color support - monochrome output only - matches termenv.Ascii */
  Ascii = 3,
  /** Alias for Ascii for compatibility */
  NoColor = 3,
}
...
export function isValidColor(value: unknown): value is ColorValue {
  ...
  if (typeof value === 'string') {
    ...
    return namedColors.includes(value as NamedColor);
  }
...
export function isValidBorderStyle(
  value: unknown
): value is BorderType | BorderStyle {
  if (
    typeof value === 'string' &&
    Object.values(BorderType).includes(value as BorderType)
  ) {
    return true;
  }
  if (typeof value === 'object' && value !== null) {
    ...
    return Object.keys(obj).every(
      key => validKeys.includes(key) && typeof obj[key] === 'string'
    );
  }
  return false;
}
