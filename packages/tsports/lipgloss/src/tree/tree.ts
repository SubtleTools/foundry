...
import { DefaultEnumerator, DefaultIndenter, RoundedEnumerator } from './enumerator';
import { createRenderer, type TreeRenderer } from './renderer';
import {
  type Children,
  type Enumerator,
  type Indenter,
...
export class Tree implements Node {
   ...
   */
  private ensureRenderer(): TreeRenderer {
    if (!this._renderer) {
   ...
   */
  private ensureParent(item: Tree): Tree {
   ...
   */
  private isChildren(value: any): value is Children {
    return (
      value &&
      typeof value === 'object' &&
      'at' in value &&
 ...
 * ```
 */
export function rootTree(root: any): Tree {
