...
import { calculateHeight, getTextWidth, wrapText } from '../layout';
import { Style } from '../style';
import { dataToMatrix, type TableData } from './data';
import { defaultStyles, HeaderRow, type StyleFunc } from './table';
import { clamp, distributeProportionally, fillArray, maxOf, median, minOf, sum } from './utils';

/**
 * Interface for column metadata used in resizing calculations.
 */
...
export class Resizer {
  ...
  private columns: ResizerColumn[] = [];
  private wrap: boolean;
  private borderColumn: boolean;
  private yPaddings: number[][]; // vertical paddings
  ...
  constructor(
    ...
    headers: string[],
    data: TableData,
    styleFunc: StyleFunc = defaultStyles,
    wrap: boolean = true,
    borderColumn: boolean = true
  ) {
    ...
    for (let i = 0; i < this.allRows.length; i++) {
      ...
      for (let j = 0; j < row.length; j++) {
        ...
        const totalHorizontalPadding =
          margins.left + margins.right + (padding?.left || 0) + (padding?.right || 0);
        ...
        const totalVerticalPadding =
          margins.top + margins.bottom + (padding?.top || 0) + (padding?.bottom || 0);
   ...
   */
  private defaultRowHeights(): number[] {
   ...
   */
  private detectTableWidth(): number {
   ...
   */
  private expandTableWidth(): DimensionResult {
   ...
   */
  private shrinkTableWidth(): DimensionResult {
    const colWidths = this.maxColumnWidths();
   ...
   */
  private shrinkBiggestColumns(colWidths: number[], veryBigOnly: boolean): void {
    while (true) {
      const totalWidth = sum(colWidths) + this.totalHorizontalBorder();
      if (totalWidth <= this.tableWidth) {
        break;
   ...
   * @param colWidths - Array of column widths to modify
   */
  private shrinkToMedian(colWidths: number[]): void {
    while (true) {
      const totalWidth = sum(colWidths) + this.totalHorizontalBorder();
      if (totalWidth <= this.tableWidth) {
        break;
   ...
   */
  private expandRowHeights(colWidths: number[]): number[] {
    ...
    for (let i = 0; i < this.allRows.length; i++) {
      ...
      for (let j = 0; j < row.length; j++) {
        ...
        const cellHeight =
          this.detectContentHeight(cell, availableWidth) + this.xPaddingForCell(i, j);
   ...
   * @param j - Column index
   */
  private xPaddingForCell(i: number, j: number): number {
    const rowPaddings = this.yPaddings[i];
