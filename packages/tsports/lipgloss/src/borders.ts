 ...
 */
export const BorderStyles: Record<BorderType, BorderStyle> & {
...
export const Borders = {
  None: { style: BorderType.None } as BorderConfig,
  Normal: { style: BorderType.Normal } as BorderConfig,
  Rounded: { style: BorderType.Rounded } as BorderConfig,
  Thick: { style: BorderType.Thick } as BorderConfig,
  Double: { style: BorderType.Double } as BorderConfig,
  Hidden: { style: BorderType.Hidden } as BorderConfig,
  ASCII: { style: BorderType.ASCII } as BorderConfig,
  Block: { style: BorderType.Block } as BorderConfig,
  OuterHalfBlock: { style: BorderType.OuterHalfBlock } as BorderConfig,
  InnerHalfBlock: { style: BorderType.InnerHalfBlock } as BorderConfig,
  Markdown: { style: BorderType.Markdown } as BorderConfig,
} as const;
...
export class BorderUtils {
  ...
  static getTopSize(border: BorderStyle): number {
    return BorderUtils.getBorderEdgeWidth(
      border.topLeft || '',
      border.top || '',
      border.topRight || ''
    );
   ...
   */
  static createPartialBorder(
    style: BorderType | BorderStyle,
    enabledSides: Array<'top' | 'right' | 'bottom' | 'left'>,
    color?: ColorValue
  ): BorderConfig {
    const config: BorderConfig = {
      style,
   ...
   */
  static horizontalBorder(
    style: BorderType | BorderStyle = BorderType.Normal,
    color?: ColorValue
  ): BorderConfig {
  ...
  static isValidBorder(border: BorderConfig): boolean {
    ...
    if (typeof border.style === 'string') {
      return Object.values(BorderType).includes(border.style as BorderType);
    }
  ...
  static mergeBorders(base: BorderConfig, override: BorderConfig): BorderConfig {
    ...
    if (topColor !== undefined) merged.topColor = topColor;

    const rightColor = override.rightColor ?? base.rightColor;
    if (rightColor !== undefined) merged.rightColor = rightColor;

    const bottomColor = override.bottomColor ?? base.bottomColor;
    if (bottomColor !== undefined) merged.bottomColor = bottomColor;

    const leftColor = override.leftColor ?? base.leftColor;
    if (leftColor !== undefined) merged.leftColor = leftColor;

    // Handle individual side background colors
    const topBackgroundColor = override.topBackgroundColor ?? base.topBackgroundColor;
    if (topBackgroundColor !== undefined) merged.topBackgroundColor = topBackgroundColor;

    const rightBackgroundColor = override.rightBackgroundColor ?? base.rightBackgroundColor;
    if (rightBackgroundColor !== undefined) merged.rightBackgroundColor = rightBackgroundColor;

    const bottomBackgroundColor = override.bottomBackgroundColor ?? base.bottomBackgroundColor;
    if (bottomBackgroundColor !== undefined) merged.bottomBackgroundColor = bottomBackgroundColor;

    const leftBackgroundColor = override.leftBackgroundColor ?? base.leftBackgroundColor;
    if (leftBackgroundColor !== undefined) merged.leftBackgroundColor = leftBackgroundColor;
...
export function NormalBorder(): BorderStyle {
  return BorderStyles[BorderType.Normal];
}

export function RoundedBorder(): BorderStyle {
  return BorderStyles[BorderType.Rounded];
}

export function BlockBorder(): BorderStyle {
  return BorderStyles[BorderType.Block];
}

export function OuterHalfBlockBorder(): BorderStyle {
  return BorderStyles[BorderType.OuterHalfBlock];
}

export function InnerHalfBlockBorder(): BorderStyle {
  return BorderStyles[BorderType.InnerHalfBlock];
}

export function ThickBorder(): BorderStyle {
  return BorderStyles[BorderType.Thick];
}

export function DoubleBorder(): BorderStyle {
  return BorderStyles[BorderType.Double];
}

export function HiddenBorder(): BorderStyle {
  return BorderStyles[BorderType.Hidden];
}

export function MarkdownBorder(): BorderStyle {
  return BorderStyles[BorderType.Markdown];
}

export function ASCIIBorder(): BorderStyle {
