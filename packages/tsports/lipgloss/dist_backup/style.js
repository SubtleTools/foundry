...
import { noColor, Color } from './color';
import { applyGoStyleWidthConstraint, calculateHeight, calculateWidth, getHorizontalPadding, getMaxTextWidth, getTextWidth, parseMarginShorthand, parsePaddingShorthand, truncateText, wrapLineGoStyle, wrapText, } from './layout';
import { defaultRenderer, Renderer } from './renderer';
import { BorderType, FontStyle, FontWeight, HorizontalAlignment, VerticalAlignment, } from './types';
/**
 ...
 */
function createOutputFromRendererOptions(options) {
...
function isValidColor(color) {
    if (color === null || color === undefined)
        return true; // Allow null/undefined (no color)
...
class SimpleColorManager {
    ...
    applyBackgroundColor(text, color) {
    ...
    applyForegroundColor(text, color) {
...
export class Style {
    constructor(options = {}, renderer) {
        ...
        if (renderer) {
            ...
            this._renderer = new Renderer(createOutputFromRendererOptions(rendererOptions));
        }
        else {
    ...
    deepCloneProperties(props) {
        ...
        for (const [key, value] of Object.entries(props)) {
            if (value === null || value === undefined) {
                ...
                // Deep clone objects like PaddingConfig, MarginConfig, BorderConfig, etc.
                cloned[key] = { ...value };
            }
            else {
    ...
    applyPlainTextStyling(str) {
        ...
        const bottomPadding = this.getNumericProperty('paddingBottom', padding.bottom);
        ...
        str = str.replace(/\r/g, '');
        // Apply borders without colors
        str = this.applyBorder(str);
        // Apply padding (not inline) - plain text only
        if (!inline) {
        ...
        // Horizontal alignment (without styling)
        {
    ...
    colorOrNil(c) {
        if (!c)
            return null;
        // Check if colors should be disabled based on renderer profile
        if (!this._renderer) {
            this._renderer = defaultRenderer();
        }
        ...
        if (typeof c === 'object' && 'color' in c && typeof c.color === 'function') {
            return c;
        }
    ...
    renderComplete(str) {
        ...
        const underlineSpaces = textDecoration.underlineSpaces !== undefined
            ? textDecoration.underlineSpaces
            : (underline ? true : false);
        const strikethroughSpaces = textDecoration.strikethroughSpaces !== undefined
        ...
        const useSpaceStyler = (underline && !underlineSpaces) ||
        ...
        if (bold)
            mainStyler = mainStyler.bold();
        if (italic)
            mainStyler = mainStyler.italic();
        if (underline)
            mainStyler = mainStyler.underline();
        if (strikethrough)
            mainStyler = mainStyler.crossOut();
        if (reverse) {
            whitespaceStyler = whitespaceStyler.reverse();
            mainStyler = mainStyler.reverse();
        }
        if (blink)
            mainStyler = mainStyler.blink();
        if (faint)
            mainStyler = mainStyler.faint();
        // Apply colors
        if (fg && fg !== noColor) {
        ...
        // Apply underline/strikethrough again for space styling
        if (underlineSpaces)
        ...
        if (!inline && width && width > 0) {
            ...
            if (wrapAt > 0) {
                // Use our wrapText function (equivalent to Go's cellbuf.Wrap)
                const { wrapText } = require('./layout');
                str = wrapText(str, wrapAt, true).join('\n');
            }
        }
        // Render core text with space handling like Go
        {
            ...
            for (let i = 0; i < lines.length; i++) {
                const line = lines[i];
                if (useSpaceStyler) {
                    // Handle each character individually for space styling
                    let renderedLine = '';
        ...
        if (!inline) {
            if (leftPadding && leftPadding > 0) {
                const paddingStr = ' '.repeat(leftPadding);
                const styledPadding = (colorWhitespace || styleWhitespace)
        ...
        if (height && height > 0) {
            const { alignTextVertical } = require('./align');
            str = alignTextVertical(str, verticalAlign, height, styleWhitespace ? whitespaceStyler : undefined);
        }
        // Horizontal alignment
        {
            const numLines = (str.match(/\n/g) || []).length;
            if (numLines > 0 || (width && width > 0)) {
                const alignStyle = (colorWhitespace || styleWhitespace) ? whitespaceStyler : undefined;
                const { alignTextHorizontal } = require('./align');
    ...
    getBooleanProperty(key, defaultValue = false) {
        switch (key) {
            case 'bold':
                return this.properties.fontWeight === 'bold' || this.properties.bold === true;
            case 'italic':
                return this.properties.fontStyle === 'italic' || this.properties.italic === true;
            case 'underline':
            ...
            case 'faint':
                return this.properties.fontWeight === 'faint' || this.properties.faint === true;
    ...
    getNumericProperty(key, fallback) {
        const value = this.properties[key] ?? fallback;
        return typeof value === 'number' ? value : undefined;
    }
    getPositionProperty(key, defaultValue = 0) {
        const alignment = this.properties[key];
    ...
    truncateToMaxWidth(str, maxWidth) {
        if (maxWidth <= 0)
            return str;
        const lines = str.split('\n');
    ...
    truncateToMaxHeight(str, maxHeight) {
        if (maxHeight <= 0)
            return str;
        const lines = str.split('\n');
        return lines.slice(0, maxHeight).join('\n');
    }
    getColorSequence(color, type) {
        if (!color)
            return '';
        // Check if colors should be disabled based on renderer profile
        if (!this._renderer) {
    ...
    applyBorder(str) {
        ...
        const { getTextWidth } = require('./layout');
        const contentWidth = lines.length > 0 ? Math.max(...lines.map(line => getTextWidth(line))) : 0;
        ...
        const colorStart = borderColor ? this.getColorSequence(borderColor, 'foreground') : '';
        ...
        if (hasTop) {
            let topLine = '';
            if (hasLeft && borderStyle.topLeft) {
                topLine += colorStart + borderStyle.topLeft + colorEnd;
            }
            if (borderStyle.top) {
                topLine += colorStart + borderStyle.top.repeat(contentWidth) + colorEnd;
            }
            if (hasRight && borderStyle.topRight) {
                topLine += colorStart + borderStyle.topRight + colorEnd;
            }
            if (topLine)
                result.push(topLine);
        }
        ...
        for (const line of lines) {
            let contentLine = '';
            if (hasLeft && borderStyle.left) {
                contentLine += colorStart + borderStyle.left + colorEnd;
            }
            contentLine += line.padEnd(contentWidth, ' '); // Ensure consistent width
            if (hasRight && borderStyle.right) {
                contentLine += colorStart + borderStyle.right + colorEnd;
            }
        ...
        if (hasBottom) {
            let bottomLine = '';
            if (hasLeft && borderStyle.bottomLeft) {
                bottomLine += colorStart + borderStyle.bottomLeft + colorEnd;
            }
            if (borderStyle.bottom) {
                bottomLine += colorStart + borderStyle.bottom.repeat(contentWidth) + colorEnd;
            }
            if (hasRight && borderStyle.bottomRight) {
                bottomLine += colorStart + borderStyle.bottomRight + colorEnd;
            }
            if (bottomLine)
    ...
    applyMargins(str, inline) {
        if (inline)
            return str;
        const margin = this.properties.margin || {};
        const topMargin = this.getNumericProperty('marginTop', margin.top) || 0;
        const rightMargin = this.getNumericProperty('marginRight', margin.right) || 0;
        const bottomMargin = this.getNumericProperty('marginBottom', margin.bottom) || 0;
        const leftMargin = this.getNumericProperty('marginLeft', margin.left) || 0;
        let result = str;
        // Apply vertical margins
        if (topMargin > 0) {
            result = '\n'.repeat(topMargin) + result;
        }
        if (bottomMargin > 0) {
            result += '\n'.repeat(bottomMargin);
        }
        ...
        if (leftMargin > 0 || rightMargin > 0) {
            ...
            result = lines.map(line => leftPad + line + rightPad).join('\n');
        }
        return result;
    }
    /**
     ...
     */
    applyDimensionConstraints(text) {
        ...
        if (this.properties.width !== undefined || this.properties.maxWidth !== undefined) {
                ...
                if (this.properties.wordWrap === false) {
                    ...
                    const truncatedLines = lines.map((line) => {
                        if (getTextWidth(line) > targetWidth) {
                            return truncateText(line, targetWidth);
                        }
                        return line;
                    });
                    ...
                    if (this.properties.horizontalAlignment !== undefined) {
                        ...
                        for (const line of lines) {
                            if (getTextWidth(line) > targetWidth) {
                                // Still need to wrap lines that are too long
                                const wrapped = wrapText(line, targetWidth, true);
                        ...
                        if (this.properties.padding) {
                            ...
                            const horizontalPadding = getHorizontalPadding(this.properties.padding);
                            const contentAreaWidth = Math.max(0, targetWidth - horizontalPadding);
                            const lines = result.split('\n');
                            const wrappedLines = [];
                            for (const line of lines) {
    ...
    applyAlignment(text) {
        // Don't skip empty text - we need to apply dimensions even for empty strings
        const hasHorizontalAlignment = this.properties.horizontalAlignment !== undefined;
        const hasVerticalAlignment = this.properties.verticalAlignment !== undefined;
        ...
        if (hasHorizontalAlignment && targetWidth !== undefined && targetWidth > 0) {
            text = AlignUtils.alignText(text, targetWidth, this.properties.horizontalAlignment);
        }
        // Apply vertical alignment if specified and we have a target height
        if (hasVerticalAlignment && targetHeight !== undefined && targetHeight > 0) {
            const lines = text.split('\n');
            ...
            const alignedLines = AlignUtils.alignVertical(lines, targetHeight, this.properties.verticalAlignment, targetWidth // Only pass width if explicitly set, otherwise undefined
            );
     ...
     */
    hasTextStyling() {
        return !!(this.properties.fontWeight === FontWeight.Bold ||
    ...
    applyMargins(text, inline) {
        ...
        if (leftMargin > 0 || rightMargin > 0) {
            const lines = result.split('\n');
            const marginSpaces = (side, count) => {
                let spaces = ' '.repeat(count);
                // Apply margin background color if specified
                if (this.properties.marginBackground && count > 0) {
                    spaces = colorManager.applyBackgroundColor(spaces, this.properties.marginBackground);
                }
                return spaces;
            };
            // Calculate maximum content width to ensure consistent line widths
            const maxContentWidth = Math.max(...lines.map((line) => getTextWidth(line)));
        ...
        if (!inline && (topMargin > 0 || bottomMargin > 0)) {
            ...
            if (this.properties.marginBackground) {
                marginLine = colorManager.applyBackgroundColor(marginLine, this.properties.marginBackground);
            }
            // Add top margin
            if (topMargin > 0) {
    ...
    backgroundColor(color) {
        if (!isValidColor(color)) {
            throw new Error(`Invalid background color value: ${JSON.stringify(color)}`);
        }
        const colorManager = SimpleColorManager.getInstance();
        const normalizedColor = colorManager.normalizeColor(color);
     ...
     * @see {@link textDecoration} For complete decoration configuration
     */
    strikethroughSpaces(enabled = true) {
        const currentDecoration = this.properties.textDecoration || {};
        return this.textDecoration({
            ...currentDecoration,
    ...
    unsetBorderBackground() {
        const currentBorder = this.properties.border || {};
        const { topBackgroundColor, rightBackgroundColor, bottomBackgroundColor, leftBackgroundColor, ...remainingBorder } = currentBorder;
        if (Object.keys(remainingBorder).length > 0) {
            return this.clone({ border: remainingBorder });
        }
        else {
    ...
    marginBackground(color) {
        if (!isValidColor(color)) {
            throw new Error(`Invalid margin background color value: ${JSON.stringify(color)}`);
        }
        const colorManager = SimpleColorManager.getInstance();
        const normalizedColor = colorManager.normalizeColor(color);
    ...
    valign(alignment) {
        switch (alignment.toLowerCase()) {
            ...
            case 'bottom':
                return this.verticalAlignment(VerticalAlignment.Bottom);
            default:
                throw new Error(`Invalid vertical alignment: ${alignment}. Valid values are: top, middle, center, bottom`);
        }
    }
    border(border, top, right, bottom, left) {
        // Handle the case where boolean parameters are provided (order: top, right, bottom, left)
        ...
        if (border && typeof border === 'object' && 'top' in border && 'left' in border) {
            const borderConfig = {
                style: border,
                // Implicit borders - all sides enabled by default (matching Go Lipgloss behavior)
                top: true,
        ...
        return this.clone({ border: border });
    }
    /**
    ...
    borderTop(enabled = true) {
        const currentBorder = this.properties.border || {};
        return this.clone({
            border: { ...currentBorder, top: enabled },
        });
    }
    /**
    ...
    borderRight(enabled = true) {
        const currentBorder = this.properties.border || {};
        return this.clone({
            border: { ...currentBorder, right: enabled },
        });
    }
    /**
    ...
    borderBottom(enabled = true) {
        const currentBorder = this.properties.border || {};
        return this.clone({
            border: { ...currentBorder, bottom: enabled },
        });
    }
    /**
    ...
    borderLeft(enabled = true) {
        const currentBorder = this.properties.border || {};
        return this.clone({
            border: { ...currentBorder, left: enabled },
        });
    }
    /**
        ...
        if (typeof transform === 'string') {
            ...
            switch (transform.toLowerCase()) {
                case 'uppercase':
                    ...
                    break;
                case 'lowercase':
                    transformFn = (text) => text.toLowerCase();
                    break;
                case 'capitalize':
                    transformFn = (text) => text.charAt(0).toUpperCase() + text.slice(1).toLowerCase();
                    break;
                default:
                    throw new Error(`Unknown transform shortcut: ${transform}. Available shortcuts: uppercase, lowercase, capitalize`);
            }
        }
        else {
     ...
     */
    applyColorWithWhitespaceSupport(text, color, isBackground) {
        const colorManager = SimpleColorManager.getInstance();
    ...
    alignHorizontal(position) {
        switch (position.toLowerCase()) {
            ...
            case 'right':
                return this.horizontalAlignment(HorizontalAlignment.Right);
            default:
                throw new Error(`Invalid horizontal alignment: ${position}. Valid values are: left, center, right`);
        }
    }
    /**
    ...
    alignVertical(position) {
        switch (position.toLowerCase()) {
            ...
            case 'bottom':
                return this.verticalAlignment(VerticalAlignment.Bottom);
            default:
                throw new Error(`Invalid vertical alignment: ${position}. Valid values are: top, middle, bottom`);
        }
    }
    ...
    SetString(...strs) {
        ...
        return this.clone({ _stringContent: content });
    }
    /**
    ...
    inherit(parent) {
        ...
        Object.keys(parentProps).forEach((key) => {
            const propKey = key;
            inheritedProps[propKey] = parentProps[propKey];
        });
        // Override with child properties where they exist
        Object.keys(childProps).forEach((key) => {
            const propKey = key;
            if (childProps[propKey] !== undefined) {
                inheritedProps[propKey] = childProps[propKey];
            }
        });
