 ...
 */
export function NewRange(start, end, style) {
 ...
 * @internal
 */
function validateRange(range, textLength) {
    if (range.start < 0) {
        throw new Error(`Range start (${range.start}) cannot be negative`);
    }
    if (range.end < range.start) {
        throw new Error(`Range end (${range.end}) cannot be before start (${range.start})`);
    }
    if (range.start > textLength) {
        throw new Error(`Range start (${range.start}) exceeds text length (${textLength})`);
    }
}
/**
 ...
 * @internal
 */
function validateNoOverlaps(ranges) {
    for (let i = 0; i < ranges.length; i++) {
        for (let j = i + 1; j < ranges.length; j++) {
            ...
            if (rangesOverlap(rangeI, rangeJ)) {
                throw new Error(`Ranges overlap: [${rangeI.start}, ${rangeI.end}) and [${rangeJ.start}, ${rangeJ.end})`);
            }
        }
    }
}
 ...
 */
function cutAnsiAware(text, start, end) {
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        if (inAnsiSequence) {
            ...
            if (/[a-zA-Z]/.test(char)) {
                ...
                // Include ANSI sequences if we're capturing OR if they come before/during the range
                // This ensures styling codes that affect our target text are preserved
                if (capturing || start === 0) {
                    result += ansiBuffer;
                }
                ansiBuffer = '';
            }
 ...
 */
export function StyleRanges(text, ...ranges) {
    if (ranges.length === 0) {
        return text;
    }
 ...
 */
export function CreateRanges(style, positions) {
 ...
 */
export function StyleWord(text, word, style, caseSensitive = false) {
    if (!word)
        return [];
