 ...
 */
function optimizeAnsiSequences(content) {
    if (!content) {
        return content;
    }
    ...
    while (hasChanges) {
        ...
        optimized = optimized.replace(/(\x1b\[38;2;\d+;\d+;\d+m)(.*?)\x1b\[0m(\x1b\[38;2;\d+;\d+;\d+m)(.*?)\x1b\[0m/g, (match, start1, content1, start2, content2) => {
        ...
        optimized = optimized.replace(/(\x1b\[48;2;\d+;\d+;\d+m)(.*?)\x1b\[0m(\x1b\[48;2;\d+;\d+;\d+m)(.*?)\x1b\[0m/g, (match, start1, content1, start2, content2) => {
            // Only merge if the color codes are identical
            if (start1 === start2) {
                return `${start1}${content1}${content2}\x1b[0m`;
            }
 ...
 */
export function JoinHorizontal(pos, ...strs) {
    if (strs.length === 0) {
        return '';
    }
    if (strs.length === 1) {
 ...
 */
export function JoinVertical(pos, ...strs) {
    if (strs.length === 0) {
        return '';
    }
    if (strs.length === 1) {
