...
export class Output implements IOutput {
  ...
  private _foregroundColor(): Color {
    ...
    // Default to light gray (matches Go behavior)
    return new ANSIColor(7);
  }
  ...
  private _backgroundColor(): Color {
    ...
    // Default to black (matches Go behavior)
  ...
  hasDarkBackground(): boolean {
    ...
    if (!c) {
      // Default to light background assumption when color cannot be determined (more conservative default)
      // in Go, NoColor -> ConvertToRGB -> Black -> HasDarkBackground returns true.
...
export function newOutput(
  ...
  return new Output(writer || process.stdout, ...opts);
}

export function withProfile(profile: Profile): OutputOption<Output> {
  return (o: Output) => {
    o.profile = profile;
  };
}

export function withColorCache(enabled: boolean): OutputOption<Output> {
  return (o: Output) => {
    o.cache = enabled;
...
export function withEnvironment(environ: Environ): OutputOption<Output> {
  return (o: Output) => {
    o.environ = environ;
  };
}

export function withTTY(assumeTTY: boolean): OutputOption<Output> {
  return (o: Output) => {
    o.assumeTTY = assumeTTY;
  };
}

export function withUnsafe(): OutputOption<Output> {
  return (o: Output) => {
    o.unsafe = true;
  };
}
 ...
 */
export function defaultOutputInstance(): Output {
  if (!defaultOutput) {
