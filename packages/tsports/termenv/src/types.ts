...
export class RGBColor implements Color {
  ...
  sequence(bg: boolean): string {
    try {
      ...
      const prefix = bg ? Background : Foreground;
      // Match Go's uint8() conversion: truncate (floor) without adding 0.5
      // Go code: uint8(f.R*255), uint8(f.G*255), uint8(f.B*255)
      // This ensures bit-perfect compatibility with Go's termenv
      // Note: This differs from go-colorful's RGB255() which rounds!
      const r = Math.floor(colorful.r * 255);
      const g = Math.floor(colorful.g * 255);
      const b = Math.floor(colorful.b * 255);
  ...
  /**
   * Parse hex color using go-colorful parser, with fallback to lenient parsing
