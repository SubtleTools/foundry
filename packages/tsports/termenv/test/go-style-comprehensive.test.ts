...
import { ANSIColor as ANSIColorClass, NoColor as NoColorType, RGBColor as RGBColorClass } from '#src/types.js';
...
describe('Go-style API', () => {
  ...
  describe('Convert function', () => {
    ...
    test('Convert handles ANSI colors', () => {
      ...
      // In all profiles, ANSI colors are preserved as-is (matches Go behavior)
    ...
    test('Convert handles ANSI256 colors', () => {
      ...
      // In all profiles except ANSI, ANSI256 colors are preserved (matches Go behavior)
      expect(Convert(TrueColor, ansi256Color)).toBe(ansi256Color);
      expect(Convert(ANSI256, ansi256Color)).toBe(ansi256Color);
      // Converted to ANSI in ANSI profile
      expect(Convert(ANSI, ansi256Color)).not.toBe(ansi256Color);
      expect(Convert(ANSI, ansi256Color)).toBeInstanceOf(ANSIColorClass);
