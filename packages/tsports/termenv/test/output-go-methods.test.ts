...
import { newOutput, withEnvironment, withProfile } from '#src/output.js';
...
describe('Output Go-compatible Methods', () => {
  test('all Go-compatible methods exist and work', () => {
    ...
    const mockEnv = new MockEnviron();
    mockEnv.setEnv('TERM', 'xterm-256color');
    mockEnv.setEnv('COLORTERM', 'truecolor');
    const output = newOutput(mockWriter as any, withProfile(Profile.TrueColor), withEnvironment(mockEnv));
